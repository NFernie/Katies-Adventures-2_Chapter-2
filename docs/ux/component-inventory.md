...
| **ChoiceChip** | Goal type, diet/kitchen flags, sex, days | 44×44 min; selected = iron invert (HyperCard press) |
...
| **SwapSheet** | Three alternatives | Bottom sheet; radio rows; confirm on tap; focus trap; 44px rows |
| **SetTable** | Live session | Set / kg / Reps / ✓ ; tabular-nums; done wash (not rainbow) |
| **WeekStrip** | 7 days | Gym days filled; tap → that day |
...
| **LockNote** | Later auth | Copy only. `role="note"`. Must not look like a disabled button |
