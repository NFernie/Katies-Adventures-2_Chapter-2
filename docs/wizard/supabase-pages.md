...
5. **Tables already exist from 0003:** paste `supabase/migrations/0004_day_plans_training_setting.sql` → **Run**. This adds `day_plans.training_setting` and `workout_sessions.setting` if they are missing (Generate writes both) and reloads the PostgREST schema cache. Safe to re-run if an earlier 0004 failed with `column s.setting does not exist`.
